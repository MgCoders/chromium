...
if ( ! function_exists( 'medicus_font_get_relative_sizes' ) ) :
    /**
     ...
     */
    function medicus_font_get_relative_sizes() {
        // Relative font sizes
        return apply_filters( 'medicus_font_relative_size', array(

        ) );
    }
endif;

if ( ! function_exists( 'medicus_css_fonts' ) ) :
    /**
     * Build the CSS rules for the custom fonts
     *
     * @return void
     */
    function medicus_css_fonts() {
        // Get relative sizes
        $percent = medicus_font_get_relative_sizes();
        ...
        $declarations = medicus_parse_font_properties( $element );
        if ( ! empty( $declarations ) ) {
            medicus_get_css()->add( array( 'selectors' => $selectors, 'declarations' => $declarations, ) );
        }
        ...
        $selectors = array( '.slide .slide-title' );
        $declarations = medicus_parse_font_properties( $element );
        if ( ! empty( $declarations ) ) {
            medicus_get_css()->add( array( 'selectors' => $selectors, 'declarations' => $declarations, ) );
        }
        ...
        $selectors = array( '.page-meta .title-page' );
        $declarations = medicus_parse_font_properties( $element );
        if ( ! empty( $declarations ) ) {
            medicus_get_css()->add( array( 'selectors' => $selectors, 'declarations' => $declarations, ) );
        }
         ...
         */
        $element = 'widgets';
        $selectors = array( '#content .title-widget' );
        $declarations = medicus_parse_font_properties( $element );
        if ( ! empty( $declarations ) ) {
            medicus_get_css()->add( array( 'selectors' => $selectors, 'declarations' => $declarations, ) );
        }
...
add_action( 'medicus_css', 'medicus_css_fonts' );

if ( ! function_exists( 'medicus_parse_font_properties' ) ) :
    /**
     ...
     */
    function medicus_parse_font_properties( $element ) {
        // css_property => sanitize_callback
        $properties = apply_filters( 'medicus_css_font_properties', array(
            'font-family'	=> 'medicus_get_font_stack',
            'font-size'		=> 'absint',
        ), $element );

        $declarations = array();
        foreach ( $properties as $property => $callback ) {
            $value = get_theme_mod( $property . '-' . $element, medicus_get_default( $property . '-' . $element ) );
            if ( false !== $value && $value !== medicus_get_default( $property . '-' . $element ) ) {
                ...
                if ( 'font-size' === $property ) {
                    ...
                    // $declarations[$property . '-rem'] = medicus_convert_px_to_rem( $sanitized_value ) . 'rem';
                } else {
...
if ( ! function_exists( 'medicus_get_font_stack' ) ) :
    /**
     ...
     */
    function medicus_get_font_stack( $font ) {
        $all_fonts = medicus_get_all_fonts();

        // Sanitize font choice
        $font = medicus_sanitize_font_choice( $font );

        // Standard font
        if ( isset( $all_fonts[ $font ]['stack'] ) && ! empty( $all_fonts[ $font ]['stack'] ) ) {
            $stack = $all_fonts[ $font ]['stack'];
        } elseif ( in_array( $font, medicus_all_font_choices() ) ) {
         ...
         */
        return apply_filters( 'medicus_font_stack', $stack, $font );
    }
endif;

if ( ! function_exists( 'medicus_get_relative_font_size' ) ) :
    /**
     ...
     */
    function medicus_get_relative_font_size( $value, $percentage ) {
        return round( (float) $value * ( $percentage / 100 ) );
    }
endif;

if ( ! function_exists( 'medicus_convert_px_to_rem' ) ) :
    /**
     ...
     * @return float             The converted value.
     */
    function medicus_convert_px_to_rem( $px, $base = 0 ) {
        return (float) $px / 10;
    }
endif;

if ( ! function_exists( 'medicus_get_font_property_option_keys' ) ) :
    /**
     ...
     */
    function medicus_get_font_property_option_keys( $property ) {
        $all_keys = array_keys( medicus_option_defaults() );
...
if ( ! function_exists( 'medicus_get_google_font_uri' ) ) :
    /**
     * Build the HTTP request URL for Google Fonts.
     *
     * @return string    The URL for including Google Fonts.
     */
    function medicus_get_google_font_uri() {
        ...
        foreach ( $font_keys as $key ) {
            $fonts[] = get_theme_mod( $key, medicus_get_default( $key ) );
        }

        // De-dupe the fonts
        $fonts         = array_unique( $fonts );
        $allowed_fonts = medicus_get_google_fonts();
        $family        = array();
        ...
        foreach ( $fonts as $font ) {
            ...
            if ( array_key_exists( $font, $allowed_fonts ) ) {
                ...
                $family[] = urlencode( $font . ':' . join( ',', medicus_choose_google_font_variants( $font, $allowed_fonts[ $font ]['variants'] ) ) );
            }
        }
        ...
        $subset = get_theme_mod( 'font-subset', medicus_get_default( 'font-subset' ) );

        if ( 'all' === $subset ) {
            $subsets_available = medicus_get_google_font_subsets();
         ...
         */
        return apply_filters( 'medicus_get_google_font_uri', esc_url( $request ) );
    }
endif;

if ( ! function_exists( 'medicus_choose_google_font_variants' ) ) :
    /**
     ...
     */
    function medicus_choose_google_font_variants( $font, $variants = array() ) {
        $chosen_variants = array();
        if ( empty( $variants ) ) {
            $fonts = medicus_get_google_fonts();
         ...
         * @param array     $variants    The variants for the font.
         */
        return apply_filters( 'medicus_font_variants', array_unique( $chosen_variants ), $font, $variants );
    }
endif;

if ( ! function_exists( 'medicus_sanitize_font_subset' ) ) :
    /**
     ...
     */
    function medicus_sanitize_font_subset( $value ) {
        if ( ! array_key_exists( $value, medicus_get_google_font_subsets() ) ) {
            $value = 'latin';
        }
         ...
         */
        return apply_filters( 'medicus_sanitize_font_subset', $value );
    }
endif;

if ( ! function_exists( 'medicus_get_google_font_subsets' ) ) :
    /**
     ...
     */
    function medicus_get_google_font_subsets() {
        /**
         ...
         */
        return apply_filters( 'medicus_get_google_font_subsets', array(
            'all'          => __( 'All', 'make' ),
...
if ( ! function_exists( 'medicus_sanitize_font_choice' ) ) :
    /**
     ...
     */
    function medicus_sanitize_font_choice( $value ) {
        if ( ! is_string( $value ) ) {
            // The array key is not a string, so the chosen option is not a real choice
            return '';
        } else if ( array_key_exists( $value, medicus_all_font_choices() ) ) {
            return $value;
        } else {
            return '';
        }
...
if ( ! function_exists( 'medicus_font_choices_placeholder' ) ) :
    /**
...
if ( ! function_exists( 'medicus_all_font_choices' ) ) :
    ...
    function medicus_all_font_choices() {
         ...
         */
        return apply_filters( 'medicus_all_font_choices', $choices );
    }
endif;

if ( ! function_exists( 'medicus_all_font_choices_js' ) ) :
    /**
     * Compile the font choices for better handling as a JSON object
     *
     * @return array
     */
    function medicus_all_font_choices_js() {
        $fonts   = medicus_get_all_fonts();
        $choices = array();
...
if ( ! function_exists( 'medicus_get_all_fonts' ) ) :
    /**
     * Compile font options from different sources.
     *
     * @return array    All available fonts.
     */
    function medicus_get_all_fonts() {
        $heading1       = array( 1 => array( 'label' => sprintf( '--- %s ---', __( 'Standard Fonts', 'make' ) ) ) );
        $standard_fonts = medicus_get_standard_fonts();
        $heading2       = array( 2 => array( 'label' => sprintf( '--- %s ---', __( 'Google Fonts', 'make' ) ) ) );
        $google_fonts   = medicus_get_google_fonts();
         ...
         */
        return apply_filters( 'medicus_all_fonts', array_merge( $heading1, $standard_fonts, $heading2, $google_fonts ) );
    }
endif;

if ( ! function_exists( 'medicus_get_standard_fonts' ) ) :
    /**
     * Return an array of standard websafe fonts.
     *
     * @return array    Standard websafe fonts.
     */
    function medicus_get_standard_fonts() {
        /**
         * Allow for developers to modify the standard fonts.
         *
         * @param array    $fonts    The list of standard fonts.
         */
        return apply_filters( 'medicus_get_standard_fonts', array(
            'serif' => array(
...
if ( ! function_exists( 'medicus_get_google_fonts' ) ) :
    /**
     * Return an array of all available Google Fonts.
     *
     * @return array    All Google Fonts.
     */
    function medicus_get_google_fonts() {
        /**
         ...
         */
        return apply_filters( 'medicus_get_google_fonts', array(
            'ABeeZee' => array(
                'label'    => 'ABeeZee',
                'variants' => array(
                    'regular',
