...
function medicus_logo() {
    $info = medicus_get_logo_information();
    ...
    if ( get_theme_mod( 'logo-retina-ready', medicus_get_default( 'logo-retina-ready' ) ) ) {
        $width /= 2;
        $height /= 2;
    }
 ...
 */
function medicus_has_logo() {
    $information = medicus_get_logo_information();
 ...
 */
function medicus_get_logo_information( $force = false ) {
    $logo_information = array();

    $logo_information['image'] = get_theme_mod( 'logo', medicus_get_default( 'logo' ) );

    if ( ! empty( $logo_information['image'] ) ) {
        $dimensions = medicus_get_logo_dimensions( $logo_information['image'], $force );
...
function medicus_get_logo_dimensions( $url, $force = false ) {
    // Build the cache key
    $key = 'medicus-' . md5( 'logo-dimensions-' . $url . WPZOOM::$themeVersion );
    ...
    if ( false === $dimensions || is_preview() || true === $force ) {
        ...
        $attachment_id = medicus_get_attachment_id_from_url( $url );
 ...
 */
function medicus_get_attachment_id_from_url( $url = '' ) {
 ...
 * @return void
 */
function medicus_display_favicons() {
    $logo_favicon = get_theme_mod( 'logo-favicon', medicus_get_default( 'logo-favicon' ) );
    if ( ! empty( $logo_favicon ) ) : ?>
        <link rel="icon" href="<?php echo esc_url( $logo_favicon ); ?>"/>
    <?php endif;
}

add_action( 'wp_head', 'medicus_display_favicons' );
